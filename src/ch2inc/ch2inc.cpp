...
int CH2Inc::Run(int argc, char** argv)
{
	...
	{
		std::cout << "Invalid platform combo specified" << std::endl;
		return -2;
	}
	...
	if (!SetupDriver())
	{
		std::cout << "Unable to setup driver" << std::endl;
		return -3;
	}
	...
	{
		std::cout << "Error during parsing: " << CH2ErrorCodeStr(m_parser.GetLastError()) << std::endl;
		return -4;
	}
	...
	m_drvfnc->SetPlatformInfo(m_sopts.info);
	m_drvfnc->SetFileInfo(m_fp);
